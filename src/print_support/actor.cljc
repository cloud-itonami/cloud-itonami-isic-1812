(ns print-support.actor
  "PrintSupportActor — the ISIC-1812 independent print-support-services
  actor as a `langgraph.graph/state-graph` (per ADR-2607011000 / CLAUDE.md
  Actors section). One graph run = one print-support operation request
...
(defn build-graph
  ...
  `print-support.store/Store`. `advisor` implements
  `print-support.advisor/Advisor` (defaults to `mock-advisor`).
